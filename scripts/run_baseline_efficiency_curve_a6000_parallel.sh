...
OUT_DIR="$ROOT/outputs/baseline_efficiency_curve_a6000"
LOG_DIR="$ROOT/outputs/verification_logs"
MASTER_LOG="$LOG_DIR/baseline_efficiency_curve_a6000_master.log"
PID_FILE="$LOG_DIR/baseline_efficiency_curve_a6000.pid"
HEARTBEAT="$LOG_DIR/baseline_efficiency_curve_a6000.heartbeat"
...
root=Path('/workspace/multi_agent/stackelberg_codepo/outputs/baseline_efficiency_curve_a6000')
method_display={
...
rows=[]
for d in sorted(root.glob('*_tok*')):
    if not d.is_dir():
        continue
    meta_path=d/'meta.json'
    meta=json.loads(meta_path.read_text(encoding='utf-8')) if meta_path.exists() else {}
    method=meta.get('method')
    token=meta.get('max_new_tokens')
    ...
    if summary_path.exists():
        try:
            ...
            row.update({
              ...
              'avg_total_tokens':s.get('avg_total_tokens'),
              'status':'completed',
            })
    ...
    if fail_path.exists():
        row['status']='failed'
        try:
            row['error']=json.loads(fail_path.read_text(encoding='utf-8')).get('returncode')
        except Exception:
            row['error']='failed'
    rows.append(row)
order={m:i for i,m in enumerate(['direct_prompt','self_consistency_3','self_repair','agentcoder'])}
rows.sort(key=lambda r:(order.get(r.get('method_key'),99), int(r.get('max_new_tokens') or 0)))
fields=['method','method_key','max_new_tokens','plan_max_new_tokens','final_passed','num_tasks','pass_rate_percent','assert_pass_rate_percent','avg_total_tokens','status','summary_path']
(root/'baseline_efficiency_curve_summary.json').write_text(json.dumps(rows, ensure_ascii=False, indent=2)+'\n', encoding='utf-8')
with (root/'baseline_efficiency_curve_summary.csv').open('w', encoding='utf-8', newline='') as f:
    w=csv.DictWriter(f, fieldnames=fields, extrasaction='ignore')
    w.writeheader(); w.writerows(rows)
with (root/'baseline_efficiency_curve_summary.md').open('w', encoding='utf-8') as f:
    f.write('| method | max_new_tokens | plan_max_new_tokens | final_passed | num_tasks | Pass Rate (%) | Assert Pass Rate (%) | Avg. Tokens | status |\n')
    f.write('| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |\n')
    for r in rows:
        def fmt(v, nd=2):
            if v is None: return ''
        ...
        f.write(f"| {r['method']} | {r.get('max_new_tokens') or ''} | {r.get('plan_max_new_tokens') or ''} | {r.get('final_passed') if r.get('final_passed') is not None else ''} | {r.get('num_tasks') if r.get('num_tasks') is not None else ''} | {fmt(r.get('pass_rate_percent'))} | {fmt(r.get('assert_pass_rate_percent'))} | {fmt(r.get('avg_total_tokens'),0)} | {r.get('status')} |\n")
print((root/'baseline_efficiency_curve_summary.md').read_text(encoding='utf-8'))
PY
}
...
validate_inputs() {
  ...
  /opt/conda/bin/python - <<'PY' || rc=1
import re
from pathlib import Path
text=Path('scripts/paper_main_baselines.py').read_text(encoding='utf-8')
for m in ['direct_prompt','self_consistency_3','self_repair','agentcoder']:
    assert m in text, m
PY
  return "$rc"
}
...
run_point() {
  ...
  local token="$2"
  local gpu="$3"
  local point="${method}_tok${token}"
  local point_dir="$OUT_DIR/$point"
  local summary="$point_dir/$method/${method}_summary.json"
  mkdir -p "$point_dir"
  /opt/conda/bin/python - <<PY
import json
from pathlib import Path
Path('$point_dir/meta.json').write_text(json.dumps({
  'method':'$method',
  'max_new_tokens':int('$token'),
  'plan_max_new_tokens':$PLAN_MAX_TOKENS,
  'gpu':int('$gpu'),
  'model':'$MODEL',
  'split':'test',
  'limit':17,
...
PY
  ...
  CUDA_VISIBLE_DEVICES="$gpu" /opt/conda/bin/python scripts/paper_main_baselines.py \
    ...
    --plan-max-new-tokens "$PLAN_MAX_TOKENS" \
    --temperature 0.2 \
...
import json
from pathlib import Path
Path('$point_dir/FAILED.json').write_text(json.dumps({'point':'$point','method':'$method','max_new_tokens':$token,'gpu':$gpu,'returncode':$rc}, indent=2)+'\n')
PY
    summarize || true
    return "$rc"
  fi
...
worker() {
  ...
  for token in "${MAX_TOKENS[@]}"; do
    run_point "$method" "$token" "$gpu" || return $?
  done
...
main() {
  log "============================================================"
  log "baseline_efficiency_curve_a6000_parallel started"
  hostname
  ...
  summarize || true
  if [ "$rc" -ne 0 ]; then
    log "baseline_efficiency_curve_a6000_parallel finished with failures"
    exit "$rc"
  fi
  log "baseline_efficiency_curve_a6000_parallel finished successfully"
}
