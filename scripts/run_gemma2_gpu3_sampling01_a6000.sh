...
LOG="$LOG_DIR/gemma2_gpu3_sampling01_a6000_master.log"
PIDFILE="$LOG_DIR/gemma2_gpu3_sampling01_a6000.pid"
HEARTBEAT="$LOG_DIR/gemma2_gpu3_sampling01_a6000.heartbeat"
CONFIG="$ROOT/configs/general_base_gemma2_9b_it_coagent_conservative_train64_gpu3_sampling01.json"
MODEL_DIR="/workspace/models/gemma-2-9b-it"
SUMMARY_DIR="$ROOT/outputs/gemma2_9b_coagent_conservative_train64_gpu3_sampling01_a6000"
...
# Gemma is larger than Mistral; wait for enough room on both sampling GPUs.
raise SystemExit(0 if free.get(0, 0) >= 30000 and free.get(1, 0) >= 30000 else 1)
PY
    rc=$?
    if [ "$rc" -eq 0 ]; then
      log "sampling GPUs ready: physical GPU0/1 >= 30000MiB"
      return 0
    fi
...
wait_dpo_gpu3() {
  CURRENT_STAGE="waiting_dpo_gpu3"
  while true; do
    "$PYTHON" - <<'PY'
import subprocess
import sys
...
for line in out.strip().splitlines():
    idx, mem = [x.strip() for x in line.split(",")]
    free[int(idx)] = int(mem)
print(f"dpo_gpu3_free_mib={free.get(3, 0)}", flush=True)
raise SystemExit(0 if free.get(3, 0) >= 44000 else 1)
PY
    rc=$?
    if [ "$rc" -eq 0 ]; then
      log "DPO GPU ready: physical GPU3 >= 44000MiB"
      return 0
    fi
    log "DPO GPU3 not ready; sleep 120s"
    sleep 120
  done
}
...
write_config() {
  ...
  CURRENT_STAGE="write_config_${train_max_length}"
...
out = Path("configs/general_base_gemma2_9b_it_coagent_conservative_train64_gpu3_sampling01.json")
cfg = json.loads(base.read_text())
cfg["run_name"] = "general_base_gemma2_9b_it_coagent_conservative_train64"
cfg["resume_stages"] = True
cfg.setdefault("paths", {})["work_dir"] = "outputs/gemma2_9b_coagent_conservative_train64_gpu3_sampling01_a6000/general_base_gemma2_9b_it_coagent_conservative_train64"
...
from pathlib import Path
status = sys.argv[1]
summary_dir = Path("outputs/gemma2_9b_coagent_conservative_train64_gpu3_sampling01_a6000")
work = summary_dir / "general_base_gemma2_9b_it_coagent_conservative_train64"
eval_summaries = sorted((work / "eval").glob("*summary.json"))
result = {
    "status": status,
    "config": "configs/general_base_gemma2_9b_it_coagent_conservative_train64_gpu3_sampling01.json",
    "sampling_gpus": [0, 1],
    "dpo_eval_gpu": 3,
...
(summary_dir / "gemma2_9b_coagent_conservative_train64_gpu3_sampling01_summary.json").write_text(json.dumps(result, indent=2, ensure_ascii=False) + "\n")
lines = ["# Gemma2 9B CoAgent Conservative Train64 GPU3/Sampling01", "", f"- status: {status}", f"- follower_adapter_exists: {result['follower_adapter_exists']}", f"- leader_adapter_exists: {result['leader_adapter_exists']}"]
...
(summary_dir / "gemma2_9b_coagent_conservative_train64_gpu3_sampling01_summary.md").write_text("\n".join(lines) + "\n")
PY
}
...
run_once() {
  ...
  cleanup_incomplete_adapters
  CURRENT_STAGE="strict_alternating_smoke_len_${train_max_length}"
  log "START strict-alternating-smoke config=$CONFIG train_max_length=$train_max_length"
...
main() {
  log "============================================================"
  log "gemma2_gpu3_sampling01_a6000"
  log "sampling_physical_gpus=0,1 dpo_eval_physical_gpu=3"
  wait_model
  wait_sampling_gpus
  wait_dpo_gpu3
  run_once 768
